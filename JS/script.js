...
function siteUrl(path) { return new URL(String(path).replace(/^\/+/, ""), SITE_ROOT).href; }
function assetUrl(path) { return siteUrl(String(path).replace(/^\.\.\//, "")); }

function getCart() {
  try {
    const cart = JSON.parse(localStorage.getItem(CART_STORAGE_KEY));
    if (!Array.isArray(cart)) return [];
    return cart.filter((item) => item && typeof item.name === "string").map((item) => ({
      ...item,
...
function setupImageFallback(root) {
  root.querySelectorAll("img").forEach((image) => image.addEventListener("error", () => {
    ...
    image.src = "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 800 600'%3E%3Crect width='800' height='600' fill='%23fde7df'/%3E%3Ctext x='400' y='310' text-anchor='middle' fill='%237b3f46' font-family='sans-serif' font-size='32'%3EImage unavailable%3C/text%3E%3C/svg%3E";
  }));
}

function priceValue(price) { return Number.parseFloat(String(price).replace(/[^0-9.]/g, "")) || 0; }
function cartSubtotal(cart = getCart()) { return cart.reduce((total, item) => total + priceValue(item.price) * item.quantity, 0); }
function formattedPrice(price) { return `₱${priceValue(price).toFixed(2)}`; }
function formattedSubtotal(cart = getCart()) { return formattedPrice(cartSubtotal(cart)); }
function isInCart(product) { return getCart().some((item) => item.name === product.name); }
...
function setActiveNavLink() {
  ...
  document.querySelectorAll("header .navbar a").forEach((link) => {
    ...
    link.classList.toggle("active", currentPath === linkPath || (currentPath === "/" && linkPath.endsWith("/index.html")));
  });
}
...
async function loadComponent(id, file) {
  ...
  try {
    ...
    target.querySelectorAll("[src^='/'], [href^='/']").forEach((element) => {
      ["src", "href"].forEach((attribute) => { if (element.hasAttribute(attribute) && element.getAttribute(attribute).startsWith("/")) element.setAttribute(attribute, siteUrl(element.getAttribute(attribute))); });
    });
...
async function renderCakeMenu() {
  ...
  try {
    ...
    containers.forEach((container) => {
      const products = cakes.filter((cake) => cake.category === container.dataset.category);
      const content = container.querySelector("[data-category-content]") || container;
      content.innerHTML = `<div class="product-grid">${products.map(productCard).join("")}</div>`;
      content.querySelectorAll("[data-add-product]").forEach((button, index) => {
        configureCartButton(button, products[index]);
        button.addEventListener("click", () => toggleProductInCart(button, products[index]));
      });
      setupImageFallback(content);
    });
  } catch (error) {
    console.error(error);
    containers.forEach((container) => { container.textContent = "Our cake menu is temporarily unavailable."; });
  }
}
...
async function setupFeaturedProductButtons() {
  ...
  try {
    ...
    buttons.forEach((button) => {
      const product = cakes.find((cake) => cake.name === button.dataset.featuredProduct);
      if (!product) return;
      configureCartButton(button, product);
      button.addEventListener("click", () => toggleProductInCart(button, product));
    });
...
async function renderProductDetails() {
  ...
  try {
    ...
    cartButton.addEventListener("click", () => toggleProductInCart(cartButton, product));
  } catch { target.innerHTML = `<p class="empty-state">This cake could not be found. <a class="text-link" href="${siteUrl("pages/cakes.html")}">Browse our cakes</a>.</p>`; }
}
...
function renderCart() {
  ...
  const cart = getCart();
  if (!cart.length) {
    target.innerHTML = '<p class="empty-state">Your cart is empty. <a class="text-link" href="/pages/cakes.html">Browse our cakes</a>.</p>';
    return;
  }
  ...
  target.querySelectorAll("[data-quantity]").forEach((input) => input.addEventListener("change", () => {
    ...
    updated[Number(input.dataset.quantity)].quantity = Math.max(1, Number(input.value) || 1);
    saveCart(updated); renderCart();
  }));
...
function setupForms() {
  document.querySelectorAll("form[data-order-form], form[data-contact-form]").forEach((form) => form.addEventListener("submit", (event) => {
    event.preventDefault();
    const draft = Object.fromEntries(new FormData(form));
    const isOrder = form.hasAttribute("data-order-form");
    localStorage.setItem(isOrder ? ORDER_DRAFT_STORAGE_KEY : CONTACT_DRAFT_STORAGE_KEY, JSON.stringify(isOrder ? { ...draft, cart: getCart() } : draft));
    const status = form.querySelector("[role=status]");
    if (status) status.textContent = isOrder ? "Your order request was saved on this device. Add a bakery email or backend to send it automatically." : "Thanks! Your message was saved on this device.";
  }));
}

document.addEventListener("DOMContentLoaded", async () => {
  await Promise.all([loadComponent("header", "/partials/header.html"), loadComponent("footer", "/partials/footer.html")]);
  updateCartCount();
  renderCakeMenu();
  setupCategoryTabs();
